...
KEY_PAIR_NAME=ci-wanna-commit-key-pair
...
#start cluster
ecs-cli up --cluster $AWS_ECS_CLUSTER_NAME --verbose -region $AWS_DEFAULT_REGION --instance-role $ROLE_NAME --size 1 --instance-type t2.micro --vpc $VPC_ID --subnets $SUBNET_ID --security-group $SECURITY_GROUP_ID --ecs-profile $PROFILE_NAME --keypair $KEY_PAIR_NAME --force
